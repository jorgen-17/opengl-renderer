...
LegacyLightingTechnique::LegacyLightingTechnique()
{
}

bool LegacyLightingTechnique::Init()
{
    if (!Technique::Init()) {
        return false;
    }

    if (!AddShader(GL_VERTEX_SHADER, "./common/shaders/legacy_lighting.vs")) {
        return false;
    }

    if (!AddShader(GL_FRAGMENT_SHADER, "./common/shaders/legacy_lighting.fs")) {
        return false;
    }
...
void LegacyLightingTechnique::SetWVP(const Matrix4f& WVP)
{
    glUniformMatrix4fv(WVPLoc, 1, GL_TRUE, (const GLfloat*)WVP.m);
}

void LegacyLightingTechnique::SetTextureUnit(unsigned int TextureUnit)
{
    glUniform1i(samplerLoc, TextureUnit);
}

void LegacyLightingTechnique::SetSpecularExponentTextureUnit(unsigned int TextureUnit)
{
    glUniform1i(samplerSpecularExponentLoc, TextureUnit);
}

void LegacyLightingTechnique::SetDirectionalLight(const DirectionalLight& Light)
{
...
void LegacyLightingTechnique::SetCameraLocalPos(const Vector3f& CameraLocalPos)
{
    glUniform3f(CameraLocalPosLoc, CameraLocalPos.x, CameraLocalPos.y, CameraLocalPos.z);
}

void LegacyLightingTechnique::SetMaterial(const Material& material)
{
...
void LegacyLightingTechnique::SetPointLights(unsigned int NumLights, const PointLight* pLights)
{
...
void LegacyLightingTechnique::SetSpotLights(unsigned int NumLights, const SpotLight* pLights)
{
